...
export default function BillSplitter({ title }) {
  ...
  const saveBill = async () => {
    try {
      await fetch("http://localhost:5000/api/bills", {
        ...
        body: JSON.stringify({ type: title, amount: total, names, split }),
      });
  ...
  return (
    <div className="bill-box">
        ...
        <div className="name-list">
          {names.map((n, i) => (
            <div key={i} className="name-tag">
              <span>{i + 1}.</span>
              <input type="text" className="name-edit-input" value={n} onChange={e => updateName(i, e.target.value)} />
      ...
      {split === "days" && (
        <>
          <label className="bill-label">Add Days Stayed (one per person)</label>
          <div className="input-add-row">
      ...
      {names.length > 0 && total > 0 && (
        <button className="add-name-btn" style={{ marginTop: "12px", width: "100%" }} onClick={saveBill}>
