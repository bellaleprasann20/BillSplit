...
export default function BillSplitter({ title }) {
  ...
  const addName = () => {
    if (nameInput.trim()) {
      ...
      setMemberCount((p) => String(parseInt(p || 0) + 1));
      setNameInput("");
    }
  };
  ...
  const addDay = () => {
    if (dayInput.trim()) {
      setDays((p) => [...p, parseInt(dayInput) || 30]);
      setDayInput("");
    }
  };
  ...
  const removeName = (i) => {
    ...
    setMemberCount((p) => String(parseInt(p || 1) - 1));
  };
  ...
  const getAmounts = () => {
    ...
    const totalDays =
      days.reduce((a, b) => a + b, 0) || 1;

    return names.map((n, i) => ({
      name: n,
      amount: (
        (((days[i] || 30) / totalDays) * total)
      ).toFixed(2),
    }));
  };
